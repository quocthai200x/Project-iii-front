<template >
    <div>
        <div v-for="(application, index) in applicationList" :key="'job-apply-' + index">
            <div v-if="application.createdBy == 1 && !application.isApproved && !application.isClose">
    ...
    methods: {
        accept(index) {
            ...
            approveByUser({ applicationId: this.applicationList[index]._id }).then(data => {
                if (data) {
                    this.applicationList[index].isApproved = data.isApproved
                    this.$q.notify({
        ...
        reject(index) {
            ...
            rejectByUser({ applicationId: this.applicationList[index]._id }).then(data => {
                if (data) {
                    this.applicationList[index].isClose = data.isClose
                    this.applicationList[index].isApproved = data.isApproved
